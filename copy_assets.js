...
const logPath = 'c:\\Users\\user\\Documents\\app\\ai_tycoon\\copy_direct_out_node.txt';
...
writeLog("Starting copy_assets.js...");

ensureDir(destDir);
for (const sub of ["characters", "avatars", "logos", "flags"]) {
  ensureDir(path.join(destDir, sub));
}

let successCount = 0;
let failCount = 0;

for (const [srcFile, [subDir, destFile]] of Object.entries(mapping)) {
  const srcPath = path.join(srcDir, srcFile);
  const destPath = subDir ? path.join(destDir, subDir, destFile) : path.join(destDir, destFile);

  try {
    if (fs.existsSync(srcPath)) {
      fs.copyFileSync(srcPath, destPath);
      writeLog(`SUCCESS: ${srcFile} -> ${destPath}`);
      successCount++;
    } else {
      writeLog(`ERROR: Source file does not exist: ${srcPath}`);
      failCount++;
    }
  } catch (err) {
    writeLog(`ERROR: Failed to copy ${srcFile} to ${destPath}: ${err.message}`);
    failCount++;
  }
}

writeLog(`Copy completed. Success: ${successCount}, Failed: ${failCount}`);
logStream.end();
